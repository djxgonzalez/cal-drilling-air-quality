---
title: "Results - Daily Annuli - Production - NO"
...
write_csv(model_output, "point_estimates/daily_annuli_production_no.csv")
model_output
```
...
model_output %>%
  ...
  labs(x = "Distance to well (km)", 
       y = "Marginal change in daily NO conc. (ppb)") +
  theme_classic()
```
...
model_output <-
  tibble(
    ...
    ci_upper  = confint(model_fit)[13:17])
write_csv(model_output, "point_estimates/daily_annuli_production_no_nowind.csv")
model_output
```

---
title: "Data Exploration: AQS NO/NOx/NOy"
...
# inputs data
aqs_nox_1999_2019 <- 
  readRDS("../../data/raw/us_epa/air_quality_system/nox/aqs_nox_1999_2019.rds")
carb_basin <- 
...
# tidies data
aqs_nox_sites <- aqs_nox_1999_2019 %>%
...
wells_buffer_production_1km <- wells_interim %>%
  ...
  st_union() %>%
  st_transform(crs_nad83)
sites_near_drilling <- aqs_nox_sites %>% 
  st_intersection(wells_buffer_drilling_10km)
sites_near_production <- aqs_nox_sites %>% 
  st_intersection(wells_buffer_production_10km)
```

## Spatial and Temporal Variation

- n = `r aqs_nox_sites %>% nrow()` AQS sites monitor for hazardous volatile organic compounds (NOx).
- n = `r aqs_nox_sites %>% st_intersection(wells_buffer_drilling_10km) %>% nrow()` NOx monitoring sites within 10 km of at least one drilling site, and n = `r aqs_nox_sites %>% st_intersection(wells_buffer_drilling_1km) %>% nrow()` are within 1 km
- n = `r aqs_nox_sites %>% st_intersection(wells_buffer_production_10km) %>% nrow()` NOx monitoring sites within 10 km of at least one drilling site, and n = `r aqs_nox_sites %>% st_intersection(wells_buffer_production_1km) %>% nrow()` are within 1 km

#### Plot. Map of NOx monitoring sites and well buffers
...
ggplot() +
  ...
  geom_sf(data = aqs_nox_sites, shape = 2) +
...
This suggests we'll have enough observations for relevant NOx (e.g., benzyne, formaldehyde) near drilling.
...
This suggests we'll also have enough observations for relevant NOx near production.
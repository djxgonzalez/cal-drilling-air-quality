---
title: "Data Exploration: AQS VOCs"
...
# inputs data
aqs_vocs_1999_2019 <- 
  readRDS("../../data/raw/us_epa/air_quality_system/vocs/aqs_vocs_1999_2019.rds")
carb_basin <- 
...
# tidies data
aqs_vocs_sites <- aqs_vocs_1999_2019 %>%
...
wells_buffer_production_1km <- wells_interim %>%
  ...
  st_union() %>%
  st_transform(crs_nad83)
sites_near_drilling <- aqs_vocs_sites %>% 
  st_intersection(wells_buffer_drilling_10km)
sites_near_production <- aqs_vocs_sites %>% 
  st_intersection(wells_buffer_production_10km)
```

## Spatial and Temporal Variation

- n = `r aqs_vocs_sites %>% nrow()` AQS sites monitor for hazardous volatile organic compounds (VOCs).
- n = `r aqs_vocs_sites %>% st_intersection(wells_buffer_drilling_10km) %>% nrow()` VOCs monitoring sites within 10 km of at least one drilling site, and n = `r aqs_vocs_sites %>% st_intersection(wells_buffer_drilling_1km) %>% nrow()` are within 1 km
- n = `r aqs_vocs_sites %>% st_intersection(wells_buffer_production_10km) %>% nrow()` VOCs monitoring sites within 10 km of at least one drilling site, and n = `r aqs_vocs_sites %>% st_intersection(wells_buffer_production_1km) %>% nrow()` are within 1 km

#### Plot. Map of VOCs monitoring sites and well buffers
...
ggplot() +
  ...
  geom_sf(data = aqs_vocs_sites, shape = 2) +
...
This suggests we'll have enough observations for relevant VOCs (e.g., benzyne, formaldehyde) near drilling.
...
This suggests we'll also have enough observations for relevant VOCs near production.
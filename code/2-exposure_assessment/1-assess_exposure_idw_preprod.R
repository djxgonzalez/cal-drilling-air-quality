### NOTE: This code is kinda sloppy and needs some work, I'll revise it -DG
...
data_wells <- di_wells %>% filter(drilled_1999_to_2019 == 1)
  ...
  for (i in c(1:length(unique(data_aqs$site_id)))) {
    ...
    # assesses exposure
      ...
      for (j in c(1:length(unique(data_monitor$month_year)))) {
        ...
        wells_in_buffer_dates1 <- select(wells_in_buffer, API14,
                                         drill_exp1_begin, drill_exp1_end) %>%
          mutate(exp_interval = interval(drill_exp1_begin, drill_exp1_end))
        
        # same for wells with long dev periods
        wells_in_buffer_dates2 <- select(wells_in_buffer, API14, 
                                         drill_exp2_begin, drill_exp2_end) %>%
          mutate(exp_interval = interval(drill_exp2_begin, drill_exp2_end))
        
        # makes timing matrix for first exposure window
        timing_matrix1  <- sapply(wells_in_buffer_dates1$exp_interval, 
                                  function(x)
                                    ...
                                    int_overlaps(interval(data_monitor$month_year[j], 
                                                 ...
                                                 x))
        # repeats for second exposure window, if it exists
        timing_matrix2  <- sapply(wells_in_buffer_dates2$exp_interval, 
                                  function(x)
                                    int_overlaps(interval(data_monitor$month_year[j], 
                                                          data_monitor$month_year[j] + 30), x))
        
        # set NAs from timing matrices to False
        timing_matrix1[is.na(timing_matrix1)]   <- F
        timing_matrix2[is.na(timing_matrix2)]   <- F
        
        # combines the two above timing matrices into one
        timing_matrix <- timing_matrix1 + timing_matrix2
  ...
  # makes empty dataframe to cpature
  ...
  write_csv(out3, path = "data/interim/aqs_exposure_preprod.csv")
